...
class DeformNet(nn.Module):
    def __init__(self, n_cat=6, nv_prior=1024, training_status = True):
    ...
    def forward(self, points, img, choose, cat_id, prior, nocs, model):
        ...
        if self.training_status:
            ...
            corr_loss = (corr_loss0 + corr_loss1 + corr_loss2) / 3
            cd_loss = (cd_loss0 + cd_loss1 + cd_loss2) / 3
            entropy_loss = (entropy_loss0 + entropy_loss1 + entropy_loss2) / 3
            deform_loss = (deform_loss0 + deform_loss1 + deform_loss2) / 3
