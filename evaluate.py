...
parser.add_argument('--model', type=str, default='models/camera.pth', help='resume from saved model')
...
if opt.data == 'val':
    result_dir = 'results/val/test'
    file_path = 'CAMERA/val_list.txt'
    cam_fx, cam_fy, cam_cx, cam_cy = 577.5, 577.5, 319.5, 239.5
else:
    result_dir = 'results/real/test'
...
def detect():
    estimator = DeformNet(opt.n_cat, opt.nv_prior, training_status=False)
    estimator.cuda()
    ...
    for path in tqdm(img_list):
        ...
        if len(valid_inst):
            ...
            t_now = time.time()
            assign_mat, deltas = estimator(f_points, f_rgb, f_choose, f_catId, f_prior, nocs=None, model=None)
            del(f_rgb)
            del(f_catId)
            inst_shape = f_prior + deltas
            assign_mat = F.softmax(assign_mat, dim=2)
            f_coords = torch.bmm(assign_mat, inst_shape)
            del(assign_mat)
            del(f_prior)
            del(deltas)
            torch.cuda.synchronize()
