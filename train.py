...
parser.add_argument('--result_dir', type=str, default='CAMERA_result', help='directory to save train results')
...
def train_net():
    ...
    # model & loss
    estimator = DeformNet(opt.n_cat, opt.nv_prior, True)
    estimator.cuda()
    ...
    for epoch in range(opt.start_epoch, opt.max_epoch + 1):
        ...
        for i, data in enumerate(val_dataloader, 1):
            ...
            sRT = sRT.cuda()
            nocs = nocs.cuda()
            assign_mat, deltas, loss, corr_loss, cd_loss, entropy_loss, deform_loss = estimator(points, rgb, choose, cat_id, prior, nocs, model)
            loss = loss.sum()
